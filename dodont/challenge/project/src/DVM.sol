...
contract DVMTrader is DVMVault {
    ...
    function sellBase(
        address to
    ) external preventReentrant returns (uint256 receiveQuoteAmount) {
        uint256 baseBalance = _BASE_TOKEN_.balanceOf(address(this));
    ...
    function sellQuote(
        address to
    ) external preventReentrant returns (uint256 receiveBaseAmount) {
        uint256 quoteBalance = _QUOTE_TOKEN_.balanceOf(address(this));
    ...
    function querySellBase(
        ...
        (receiveQuoteAmount, ) = PMMPricing.sellBaseToken(
            getPMMState(),
            payBaseAmount
        );
    ...
    function querySellQuote(
        ...
        (receiveBaseAmount, ) = PMMPricing.sellQuoteToken(
            getPMMState(),
            payQuoteAmount
        );
...
contract DVMFunding is DVMVault {
        ...
        returns (uint256 shares, uint256 baseInput, uint256 quoteInput)
    {
        ...
        if (totalSupply == 0) {
            ...
            // case 3. normal case
            uint256 baseInputRatio = DecimalMath.divFloor(
                baseInput,
                baseReserve
            );
            uint256 quoteInputRatio = DecimalMath.divFloor(
                quoteInput,
                quoteReserve
            );
            uint256 mintRatio = quoteInputRatio < baseInputRatio
                ? quoteInputRatio
                : baseInputRatio;
    ...
    function init(
        ...
        name = string(
            abi.encodePacked(
                suffix,
                connect,
                addressToShortString(address(this))
            )
        );
        symbol = "DLP";
